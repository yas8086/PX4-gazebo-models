...
索引即统一契约索引（= 执行器囊号 Blower1~4/Valve1~4 = 传感器槽序 = ballast_setpoint 数组下标）。
布线契约: 囊1=左副, 囊2=左主, 囊3=右主, 囊4=右副（左右以站在艇尾朝艇头看为准）。

| 索引(囊号) | 名称 | Y坐标(m) | 相对重心Y(m) | 位置 |
|------|------|---------|-------------|------|
| 0 (囊1) | LO 左外副囊 = 左副囊 | +8.6 | +8.6 | 左侧外侧 |
| 1 (囊2) | LI 左内主囊 = 左主囊 | +3.3 | +3.3 | 左侧内侧 |
| 2 (囊3) | RI 右内主囊 = 右主囊 | -3.3 | -3.3 | 右侧内侧 |
| 3 (囊4) | RO 右外副囊 = 右副囊 | -8.6 | -8.6 | 右侧外侧 |
